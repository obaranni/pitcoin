...
class Blockchain:
    ...
    def calculate_utxo(self, block, utxo_file_path=UTXO_FILE, del_old=0):
        self.create_utxo_if_not_exit(utxo_file_path)
        if del_old:
    ...
    def create_chain(self):
        ...
        self.save_block(block, BLOCKCHAIN_DB, UTXO_FILE, del_old=1)
        ...
        try:
            ...
            while True:
                ...
                if resp is None or str(resp.json()).find("error") != -1:
                    break
                block = convert_block_from((resp.json())['block'])
                self.save_block(block, TMP_BLOCKCHAIN_DB, TMP_UTXO_FILE)
                i += 1
            if self.is_valid_chain(TMP_BLOCKCHAIN_DB, TMP_UTXO_FILE):
    ...
    def is_valid_chain(self, db_file, utxo_file_path):
        self.create_db_if_not_exist(db_file)
        i = 0
        while True:
            block, block_json = self.get_block_by_id(i, db_file)
            print(block_json)
            if not block:
                break
            utxo = self.get_utxo("address", utxo_file_path)
            if utxo:
                utxo_flags = [[i, 0] for i in utxo]
            else:
                utxo_flags = False
            print(utxo_flags)
            for tx in block.transactions:
                if tx != block.transactions[-1]:
                    if not self.verify_tx(tx, utxo_flags):
                        return False
            self.calculate_utxo(block, TMP_UTXO_FILE)
            i += 1
        return True
    ...
    def save_block(self, block, db_file, utxo_file_path, del_old=0):
        self.blocks.append(block)
        self.blocks = [self.blocks[-1]]
        self.save_blocks(db_file)
        self.calculate_utxo(self.blocks[-1], utxo_file_path, del_old)
